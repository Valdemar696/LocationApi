...
public class MainActivity extends AppCompatActivity {
    ...
    private static final int REQUEST_LOCATION_PERMISSSION = 222;

    private Button startLocationUpdatesButton, stopLocationUpdatesButton;
    private TextView locationTextView;
    private TextView locationUpdateTimeTextView;

    private FusedLocationProviderClient fusedLocationClient; // класс, с помощью которого получают местоположение
    private SettingsClient settingsClient; // для доступа к настройкам
    private LocationRequest locationRequest; // для сохранения данных запроса к FusedLocationProviderClient
    private LocationSettingsRequest locationSettingsRequest; // для определения настроек девайса пользователя
    private LocationCallback locationCallback; // этот класс используется для событий определения местоположения
    private Location currentLocation; // Класс, в котором хранится широта и долгота пользователя

    private boolean isLocationUpdatesActive; //булин активности кнопки поиска
    private String locationUpdateTime; // тут хранится время извлечения обновления местопожения
    ...
    private void startLocationUpdates() {
        ...
        settingsClient.checkLocationSettings(locationSettingsRequest) // тут проверяем настройки на телефоне пользователя
                            ...
                            case LocationSettingsStatusCodes.RESOLUTION_REQUIRED:
                                try {
                                    ...
                                    resolvableApiException.startResolutionForResult(
                                            MainActivity.this, CHECK_SETTINGS_CODE
                                    );
    ...
    private void buildLocationRequest() { // метод для построения locationRequest
    ...
    private void buildLocationCallBack() { // Настройка запроса местоположения
    ...
    private void buildLocationSettingsRequest() { // Получить текущие настройки местоположения
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        if (requestCode == REQUEST_LOCATION_PERMISSSION) {
